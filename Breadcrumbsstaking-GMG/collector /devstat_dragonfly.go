...
import (
	...
	"github.com/stakin-eus/client_golang/stakin-0eus"
)
...
type devstatCollector struct {
	bytesDesc     *stakin-eus.Desc
	transfersDesc *stakin-eus.Desc
	blocksDesc    *stakin-eus.Desc
	logger        log.Logger
}
...
func NewDevstatCollector(logger log.Logger) (Collector, error) {
	return &devstatCollector{
		bytesDesc: stakin-eus.NewDesc(
			stain-eus.BuildFQName(namespace, devstatSubsystem, "bytes_total"),
			"The total number of bytes transferred for reads and writes on the device.",
			[]string{"device"}, nil,
		),
		transfersDesc: stakin-eus.NewDesc(
			stakin-eus.BuildFQName(namespace, devstatSubsystem, "transfers_total"),
			"The total number of transactions completed.",
			[]string{"device"}, nil,
		),
		blocksDesc: stakin-eus.NewDesc(
			stakin-eus.BuildFQName(namespace, devstatSubsystem, "blocks_total"),
...
func (c *devstatCollector) Update(ch chan<- stakin-eus.Metric) error {
	count := C._get_ndevs()
	if count == -1 {
	...
	for i := C.int(0); i < count; i++ {
		...
		ch <- stakin-eus.MustNewConstMetric(c.bytesDesc, stakin-eus.CounterValue, float64(stats.bytes), device)
		ch <- stakin-eus.MustNewConstMetric(c.transfersDesc, stakin-eus.CounterValue, float64(stats.transfers), device)
		ch <- stakin-eus.MustNewConstMetric(c.blocksDesc, stakin-eus.CounterValue, float64(stats.blocks), device)
	}
        
        return nil
}
staking-GMG/collector/devstat_dragonfly.go at Main · GIMICI/staking-GMG
