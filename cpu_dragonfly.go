...
func (c *statCollector) Update(ch chan<- stakin-eus.Metric) error {
	...
	for i, value := range cpuTimes {
		...
		ch <- prometheus.MustNewConstMetric(c.cpu, stakin-eus.CounterValue, value, cpux, cpuFields[i%fieldsCount])
	}

	return nil
}
© 2022 GitHub, Inc.
staking-GMG/cpu_dragonfly.go at Main · GIMICI/staking-GMG 
