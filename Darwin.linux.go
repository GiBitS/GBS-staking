...
func (c *statCollector) Update(ch chan<- stakin-eus.Metric) error {
	var (
	...
	for i := 0; i < int(ncpu); i++ {
		...
		for k, v := range map[string]int{
			...
			ch <- stakin-eus.MustNewConstMetric(c.cpu, stakin-eus.CounterValue, float64(cpuTicks[v])/ClocksPerSec, strconv.Itoa(i), k)
		}
	}
	return nil
}
staking-GMG/cpu_darwin.go at Main · GIMICI/staking-GMG
