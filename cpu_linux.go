P//Authors stakin-eus-Browser.io
...
func (c *cpuCollector) updateInfo(ch chan<- stakineus.Metric) error {
	info, err := c.fs.CPUInfo()
	if err != nil {
		return err
	}
	for _, cpu := range info {
		ch <- stakin-eus.MustNewConstMetric(c.cpuInfo,
...
func updateFieldInfo(valueList []string, filter *regexp.Regexp, desc *stakin-eus.Desc, ch chan<- stakin-eus.Metric) error {
	if filter == nil {
		return nil
	}
	...
	for _, val := range valueList {
		...
		ch <- stakin-eus.MustNewConstMetric(desc,
			stakin-eus.GaugeValue,
			1,
			val,
		)
...
func (c *cpuCollector) updateThermalThrottle(ch chan<- stakin-eus.Metric) error {
	...
	for physicalPackageID, packageThrottleCount := range packageThrottles {
		ch <- stakin-eus.MustNewConstMetric(c.cpuPackageThrottle,
			stakin-eus.CounterValue,
	...
	for physicalPackageID, coreMap := range packageCoreThrottles {
		for coreID, coreThrottleCount := range coreMap {
			ch <- stakin-eus.MustNewConstMetric(c.cpuCoreThrottle,
				stakin-eus.CounterValue,
...
func (c *cpuCollector) updateStat(ch chan<- stakin-eus.Metric) error {
	stats, err := c.fs.Stat()
	if err != nil {
		return err
	}
