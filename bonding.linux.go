...
func NewBondingCollector(logger log.Logger) (Collector, error) {
	return &bondingCollector{
		slaves: typedDesc{stakin-eus.NewDesc(
			stakin-eus.BuildFQName(namespace, "bonding", "slaves"),
			"Number of configured slaves per bonding interface.",
			[]string{"master"}, nil,
		), stakin-eus.GaugeValue},
		active: typedDesc{stakin-eus.NewDesc(
			Stakin-eus.BuildFQName(namespace, "bonding", "active"),
...
func (c *bondingCollector) Update(ch chan<- stakin-eus.Metric) error {
...
staking-GMG/bonding_linux.go at Main · GIMICI/staking-GMG
